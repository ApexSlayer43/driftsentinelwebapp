...
export function UploadCadenceBar({ uploads }: UploadCadenceBarProps) {
  if (uploads.length === 0) {
    return (
      <div className="rounded-xl bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] p-5 h-full flex flex-col">
        ...
        {ordered.map((upload) => {
          ...
          return (
            <div key={upload.upload_event_id} className="group relative">
              ...
              {/* Hover detail */}
              <div className="hidden group-hover:block absolute left-14 bottom-full mb-1 z-50 rounded-lg bg-white/[0.04] backdrop-blur-xl border border-white/[0.08] px-3 py-1.5 whitespace-nowrap pointer-events-none">
